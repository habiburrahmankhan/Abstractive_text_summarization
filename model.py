...
class SummarizationModel(object):
  ...
  def _add_encoder(self, encoder_inputs, seq_len):
    ...
    with tf.variable_scope('encoder'):
      cell_fw = tensorflow.contrib.rnn.LSTMCell(self._hps.hidden_dim, initializer=self.rand_unif_init, state_is_tuple=True)
      cell_bw = tensorflow.contrib.rnn.LSTMCell(self._hps.hidden_dim, initializer=self.rand_unif_init, state_is_tuple=True)
  ...
  def _reduce_states(self, fw_st, bw_st):
    ...
    with tf.variable_scope('reduce_final_st'):
      ...
      return tensorflow.contrib.rnn.LSTMStateTuple(new_c, new_h) # Return new cell and state
  ...
  def _add_decoder(self, inputs):
    ...
    """
    hps = self._hps
    cell = tensorflow.contrib.rnn.LSTMCell(hps.hidden_dim, state_is_tuple=True, initializer=self.rand_unif_init)
      ...
      if hps.mode in ['train', 'eval']:
        ...
        with tf.variable_scope('loss'):
          if FLAGS.pointer_gen:
            ...
            self._loss = tensorflow.contrib.seq2seq.sequence_loss(tf.stack(vocab_scores, axis=1), self._target_batch, self._dec_padding_mask) # this applies softmax internally
  ...
  def run_encoder(self, sess, batch):
    ...
    dec_in_state = tensorflow.contrib.rnn.LSTMStateTuple(dec_in_state.c[0], dec_in_state.h[0])
  ...
  def decode_onestep(self, sess, batch, latest_tokens, enc_states, dec_init_states, prev_coverage):
    ...
    new_dec_in_state = tensorflow.contrib.rnn.LSTMStateTuple(new_c, new_h)
    ...
    new_states = [tensorflow.contrib.rnn.LSTMStateTuple(results['states'].c[i, :], results['states'].h[i, :]) for i in range(beam_size)]
